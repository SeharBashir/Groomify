...
const SalonSignupScreen = ({ navigation, route }) => {
  ...
  const handleImageSelection = async (setter, errorKey) => {
  ...
  const handleSalonSignup = async () => {
    ...
    try {
      const salonLogoUrl = await uploadImageToCloudinary(salonLogo);
      const authImageUrl = await uploadImageToCloudinary(authImage);

      const newSalonRef = ref(db, `salons/${Date.now()}`);
      await set(newSalonRef, {
        salonName,
        ownerName,
        email,
        phoneNumber,
      ...
      Alert.alert('Success', 'Salon signup submitted. Waiting for admin approval.');
      navigation.goBack();
    } catch (error) {
      console.error('Signup error:', error);
      Alert.alert('Error', 'Failed to register salon. Please try again.');
    }
  };
    ...
    >
      ...
      >
        ...
        <TouchableOpacity 
          ...
          style={[styles.inputContainer, errors.startTime && styles.errorInput]}
        >
          <Ionicons name="time" size={20} color="#00665C" style={styles.icon} />
        ...
        <TouchableOpacity 
          ...
          style={[styles.inputContainer, errors.endTime && styles.errorInput]}
        >
          <Ionicons name="time" size={20} color="#00665C" style={styles.icon} />
        ...
        <TouchableOpacity 
          style={[styles.uploadButton, errors.authImage && styles.errorInput]} 
          onPress={() => handleImageSelection(setAuthImage, 'authImage')}
        >
          <Ionicons name="image" size={20} color="#00665C" style={styles.icon} />
          <Text style={styles.buttonText}>Upload Authentication Image *</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          style={[styles.uploadButton, errors.salonLogo && styles.errorInput]} 
          onPress={() => handleImageSelection(setSalonLogo, 'salonLogo')}
        >
          <Ionicons name="image" size={20} color="#00665C" style={styles.icon} />
          <Text style={styles.buttonText}>Upload Salon Logo *</Text>
        </TouchableOpacity>
