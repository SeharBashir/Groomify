...
//       const bookingRef = ref(realtimeDb, `bookings/${user.uid}`);
//       const newBookingRef = push(bookingRef); // 🔥 Generate unique booking ID
...
//       // ✅ Correct path for booking as per Firebase Rules
//       const bookingRef = ref(realtimeDb, `bookings/${uid}`);
...
//       // ✅ Save user action (only logs or allowed analytics paths)
//       try {
...
//       Alert.alert('Success', 'Booking is confirmed successfully!');
//     } catch (error) {
//       console.error('Booking Permission Error:', error);
//       Alert.alert('Error', 'Permission denied: You are not allowed to perform this action.');
//     } finally {
//       setBookingLoading(false);
//     }
//   };
...
const BookServiceScreen = () => {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      // Logging is optional, errors won't show in UI
      try {
        await saveUserAction(
          uid,
          serviceId,
      ...
      // No alert here, error only in console
    } finally {
      setBookingLoading(false);
    }
  };
