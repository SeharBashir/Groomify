...
const HistoryScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      try {
        const salonsSnapshot = await get(ref(db, "salons"));
        const salonsData = salonsSnapshot.exists() ? salonsSnapshot.val() : {};
        ...
        const unsubscribe = onValue(bookingsRef, (snapshot) => {
          if (snapshot.exists()) {
            ...
            const userBookings = Object.entries(data)
              .map(([key, booking]) => {
                const matchedSalon = Object.values(salonsData).find(
                  salon => salon.ownerId === booking.ownerId
                );
  ...
  const handleCancelBooking = async (booking) => {
    if (["Accepted", "Completed"].includes(booking.status)) {
      return Alert.alert("Cannot Cancel", "Your booking is already accepted or completed.");
    }

    Alert.alert("Confirm Cancellation", "Are you sure you want to cancel?", [
      { text: "No", style: "cancel" },
      {
        text: "Yes",
        onPress: async () => {
          try {
            await update(ref(db, `bookings/${auth.currentUser.uid}/${booking.bookingId}`), {
              status: "Cancelled",
            });
            ...
            {selectedBooking && (
              <>
                ...
                <View style={[
                  ...
                  <Text style={styles.modalStatusText}>STATUS: {selectedBooking.status?.toUpperCase()}</Text>
                </View>
              </>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  headerText: { fontSize: 24, fontWeight: "bold", color: "#00665C" },
  emptyContainer: { flex: 1, justifyContent: "center", alignItems: "center" },
  emptyText: { marginTop: 16, fontSize: 16, color: "#555" },
  flatListContent: { paddingTop: 20 },
  bookingCard: {
  ...
  cancelledCard: { backgroundColor: "#FFE5E5" },
  ...
  completedBadge: { backgroundColor: "green" },
  cancelledBadge: { backgroundColor: "#CC0000" },
  acceptedBadge: { backgroundColor: "#00665C" },
  bookingRow: { flexDirection: "row", alignItems: "center", marginVertical: 2 },
  bookingDetail: { marginLeft: 8, fontSize: 14 },
  cancelButton: {
