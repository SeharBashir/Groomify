...
const LoginForm = () => {
  ...
  const { role } = route.params || {}; // Extract role from params
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Navigate to the next screen with role
      navigation.navigate("BeautyQuestionnaire", { role: userRole });
    ...
    <ImageBackground source={require("../../assets/images/sinpic.jpg")} style={styles.backgroundImage}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.overlay}>
        ...
        <View style={styles.formContainer}>
          ...
          {role && <Text style={styles.roleText}>Logging in as: {role.toUpperCase()}</Text>}
          ...
          <View style={styles.linkContainer}>
            <Text style={styles.text}>Don't have an account? </Text>
            <TouchableOpacity onPress={() => navigation.navigate("userSignupScreen", { role })}>
              <Text style={styles.linkText}>Sign up</Text>
            </TouchableOpacity>
          </View>
        </View>
