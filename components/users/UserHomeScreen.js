...
const UserHomeScreen = () => {
  ...
  // Fetch services
  useEffect(() => {
    if (salonList.length === 0 || userPreferences.selectedServices.length === 0) return;
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          const filteredServices = servicesArray.filter((service) => {
            ...
            const salonType = salon.salonType.toLowerCase();
            const prefSalonType = userPreferences.salonType.toLowerCase();

            if (userPreferences.salonType === "Both") return true;
            if (prefSalonType === salonType) return true;
            if (prefSalonType === "home salon" && salonType.includes("home")) return true;
            if (prefSalonType === "in salon" && salonType.includes("salon")) return true;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.categoriesContainer}>
        <View style={styles.categoriesRow}>
          ...
          <TouchableOpacity style={styles.categoryCard} onPress={() => navigation.navigate("Nails", { category: "Nails" })}>
            <FontAwesome name="paint-brush" size={40} color="#388E3C" />
            <Text style={styles.categoryText}>Nails</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.categoryCard} onPress={() => navigation.navigate("Facial", { category: "Facial" })}>
        ...
        <View style={styles.categoriesRow}>
          <TouchableOpacity style={styles.categoryCard} onPress={() => navigation.navigate("Coloring", { category: "Coloring" })}>
            <FontAwesome name="paint-brush" size={40} color="#9575CD" />
            <Text style={styles.categoryText}>Coloring</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.categoryCard} onPress={() => navigation.navigate("Spa", { category: "Spa" })}>
            <FontAwesome name="spa" size={40} color="#80CBC4" />
            <Text style={styles.categoryText}>Spa</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.categoryCard} onPress={() => navigation.navigate("Waxing", { category: "Waxing" })}>
            <FontAwesome name="tint" size={40} color="#8B4513" />
            <Text style={styles.categoryText}>Waxing</Text>
          </TouchableOpacity>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  header: { padding: 30, backgroundColor: "#004D40", borderBottomLeftRadius: 60, borderBottomRightRadius: 60, paddingBottom: 110 },
  headerTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  profileContainer: { flexDirection: "row", alignItems: "center", justifyContent: "center" },
  profileIcon: { width: 60, height: 60, borderRadius: 20, marginRight: 10, marginTop: 70 },
  username: { fontSize: 24, color: "#fff", fontWeight: "bold", marginTop: 78 },
  headerText: { fontSize: 30, color: "#fff", fontWeight: "bold", marginTop: 30, textAlign: "center" },
  sectionTitle: { fontSize: 25, fontWeight: "bold", color: "#004D40", marginLeft: 20, marginBottom: 10 },
  scrollContainer: { paddingHorizontal: 20 },
  serviceCard: { width: 130, height: 150, marginRight: 15, backgroundColor: "#fff", borderRadius: 15, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4, elevation: 5 },
  serviceImage: { width: "100%", height: 100, borderTopLeftRadius: 15, borderTopRightRadius: 15 },
  serviceDetails: { padding: 5 },
  serviceName: { fontSize: 14, fontWeight: "bold", color: "#333", textAlign: "center" },
  servicePrice: { fontSize: 12, color: "red", marginTop: 2, textAlign: "center" },
  categoriesContainer: { paddingHorizontal: 20, marginBottom: 90 },
  categoriesRow: { flexDirection: "row", justifyContent: "space-between", marginBottom: 15 },
  categoryCard: { width: "30%", height: 90, backgroundColor: "#fff", borderRadius: 15, justifyContent: "center", alignItems: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4, elevation: 5, borderColor: "#00665C", borderWidth: 3 },
  categoryText: { fontSize: 14, fontWeight: "bold", color: "#333" },
  bottomNavigation: { flexDirection: "row", justifyContent: "space-around", alignItems: "center", position: "absolute", bottom: 0, left: 0, right: 0, backgroundColor: "#004D40", paddingVertical: 10, height: 60 },
  bottomNavItem: { alignItems: "center", flex: 1 },
  bottomNavIcon: { width: 24, height: 24, tintColor: "#fff", marginBottom: 5 },
  bottomNavText: { fontSize: 12, fontWeight: "bold", color: "#fff" },
