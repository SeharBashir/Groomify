    ...
    const unsubscribeServices = onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          // ✅ Filter Makeup Services
          const makeupServices = servicesArray.filter(
            (service) =>
              service.serviceName &&
              service.serviceName.trim().toLowerCase().includes('makeup')
          );
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <Text style={styles.emptyMessage}>No Makeup services available.</Text>
      )}
    </View>
  );
};
